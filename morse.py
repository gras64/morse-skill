...
def write_signal(wavef, duration, speed=1, volume=0, rate=44100.0,
                 frequency=1240.0, speedfactor=1):
        """
...
def morse_to_wav(text, speed, file_=None):
    ...
    for char in text:
        ...
        write_signal(wav, 0.2, speed, volume=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', help="speedfactor is *duration ", action='store',
                            type=float)   
    ...
    if args.o == 'sound':
        ...
        f = morse_to_wav(morse, speed=args.s, file_=args.f)
        if not args.f:
            play(f)
    else:
        text = ' '.join(args.args)
